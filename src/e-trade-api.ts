...
export class ETrade {
  ...
  constructor(options?: Partial<ETradeOptions>) {
    ...
    this.throttle = new Throttle(
      ...
      this.settings.connectionLimitPeriod,
      this.settings.errorOnConnectionLimit
    );
    ...
    return merge(
      {
        method: "GET",
        baseURL:
          this.settings.mode === "prod"
            ? this.settings.urls.prod
            : this.settings.urls.dev,
        headers: {
  ...
  private signRequest(
    ...
    token?: boolean | { key: string; secret: string },
    omit: boolean = false
  ): void {
    const options = merge({}, request);

    options.url = [options.baseURL || "", options.url || ""].join("");
    ...
    const authorization: any = this.oauth.authorize(
      options,
      token === false ? undefined : token
    );
  ...
  async getAccessToken(
    options: GetAccessTokenRequest
  ): Promise<GetAccessTokenResponse> {
    const requestOptions = this.getBasicRequest();

    delete requestOptions.baseURL;

    requestOptions.url = [this.settings.urls.oauth, "access_token"].join("");
  ...
  async renewAccessToken(options: RenewAccessTokenRequest) {
    ...
    requestOptions.url = [this.settings.urls.oauth, "renew_access_token"].join(
      ""
    );
  ...
  async requestToken(): Promise<RequestTokenResponse> {
    ...
    requestOptions.url = [this.settings.urls.oauth, "request_token"].join("");
    ...
    const results = parseQueryString(
      await this.request<string>(requestOptions)
    );
  ...
  async revokeAccessToken(options: RenewAccessTokenRequest) {
    ...
    requestOptions.url = [this.settings.urls.oauth, "revoke_access_token"].join(
      ""
    );
  ...
  async changePreviewedOrder({
    accountIdKey,
    orderId,
    orderType,
    clientOrderId,
    order,
  }: ChangePreviewedOrderRequest): Promise<PreviewOrderResponse> {
    const requestOptions = this.getBasicRequest({
      method: "PUT",
  ...
  async deleteAlert(alertId: number | number[]): Promise<DeleteAlertResponse> {
    const requestOptions = this.getBasicRequest({
      method: "DELETE",
      url: `user/alerts/${
        typeof alertId === "number" ? alertId : alertId.join(",")
      }.json`,
    });
  ...
  async getOptionChains({
    ...
    chainType = "CALLPUT",
    priceType = "ATNM",
  }: GetOptionChainsRequest): Promise<GetOptionChainsResponse> {
  ...
  async getOptionExpireDates({
    symbol,
    expiryType,
  }: GetOptionExpireDatesRequest): Promise<ExpirationDate[]> {
    ...
    return (await this.request<any>(requestOptions)).OptionExpireDateResponse
      .ExpirationDate;
  }
  ...
  async getQuotes({
  ...
  }: GetQuotesRequest): Promise<QuoteData> {
    ...
    const requestOptions = this.getBasicRequest({
      url: `market/quote/${
        typeof symbols === "string" ? symbols : symbols.join(",")
      }.json`,
      data: data,
    });
  ...
  async listAccounts(): Promise<Account[]> {
    ...
    return (await this.request<any>(requestOptions)).AccountListResponse
      .Accounts.Account;
  }

  async listAlertDetails({
    alertId,
    htmlTags = false,
  }: ListAlertDetailsRequest): Promise<AlertDetails> {
    const requestOptions = this.getBasicRequest({
  ...
  async listTransactionDetails({
    ...
    transactionId,
    storeId,
  }: ListTransactionDetailsRequest): Promise<Transaction> {
  ...
  async placeChangedOrder({
    ...
    clientOrderId,
    previewIds,
  }: PlaceChangedOrderRequest): Promise<PlaceOrderResponse> {
    const requestOptions = this.getBasicRequest({
      method: "PUT",
  ...
  async viewLotsDetails({
    accountIdKey,
    positionId,
  }: ViewLotsDetailsRequest): Promise<ViewLotsDetailsResponse> {
    const requestOptions = this.getBasicRequest({
  ...
  async viewPortfolio({
    ...
    lotsRequired = false,
    view = "QUICK",
  }: ViewPortfolioRequest): Promise<Portfolio[]> {
...
export type sortBy =
  ...
  | "TYPE_CODE"
  | "DISPLAY_SYMBOL"
  | "AFTER_HOURS_PCTCHANGE"
  | "PRE_MARKET_PCTCHANGE"
...
export type egQual =
  | "EG_QUAL_UNSPECIFIED"
  | "EG_QUAL_QUALIFIED"
  | "EG_QUAL_NOT_IN_FORCE"
  | "EG_QUAL_NOT_A_MARKET_ORDER"
  | "EG_QUAL_NOT_AN_ELIGIBLE_SECURITY"
  | "EG_QUAL_INVALID_ORDER_TYPE"
  | "EG_QUAL_SIZE_NOT_QUALIFIED"
  | "EG_QUAL_OUTSIDE_GUARANTEED_PERIOD"
  | "EG_QUAL_INELIGIBLE_GATEWAY"
  | "EG_QUAL_INELIGIBLE_DUE_TO_IPO"
  | "EG_QUAL_INELIGIBLE_DUE_TO_SELF_DIRECTED"
